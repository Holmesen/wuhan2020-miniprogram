// miniprogram/pages/statisticsCity/statisticsCity.js
...
Page({
  ...
  columnShow: function (canvasId, cate, data, title, name, width, height) {
    if (cate.length > 8) { cate = cate.splice(0, 8) }
    if (data.length > 8) { data = data.splice(0, 8) }
    let column = {
      canvasId: canvasId,
      type: 'column',
      animation: true,
    ...
    wx.cloud.callFunction({
      ...
      if (!!res && !!res.result) {
        ...
        if(this.data.type==='in'){
          ...
          this.columnShow('canvas1', detail_out_city, detail_out_value, '全国热门迁出城市', '迁出率 (该城市迁出人数 / 全国迁出总人数)', this.data.screenW, this.data.screenW * 0.618)
        }
      }
      wx.hideLoading()
    }).catch(err => {
  ...
  onLoad: function (options) {
    ...
    wx.setNavigationBarTitle({ title: options.type === 'in' ? '全国迁入城市' :'全国迁出城市'})
    this.setData({
